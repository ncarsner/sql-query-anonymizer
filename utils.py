...
def tokenize_sql(query: str) -> List[Token]:
    ...
    token_specification = [
        ...
        (TokenType.TABLE_ALIAS, r"(?<=FROM\s)\w+\s(\w+)"  # Alias after table
                               r"|(?<=\bJOIN\s)\w+\s(\w+)"),  # Alias after JOIN
...
def _post_process_tokens(tokens: List[Token]) -> List[Token]:
    ...
    for i, token in enumerate(tokens):
        ...
        if token.type == TokenType.TABLE and i + 1 < len(tokens):
            ...
            if (next_token.type == TokenType.IDENTIFIER and 
                next_token.value.upper() not in SQL_KEYWORDS and
                next_token.value.upper() not in ALL_SQL_FUNCTIONS):
                table_aliases.add(next_token.value.lower())
                tokens[i + 1] = Token(TokenType.TABLE_ALIAS, next_token.value, next_token.space)
        
        # Detect column aliases (after AS keyword)
        elif (token.type == TokenType.KEYWORD and 
              token.value.upper() == 'AS' and 
              i + 1 < len(tokens)):
            next_token = tokens[i + 1]
            if (next_token.type == TokenType.IDENTIFIER and
                next_token.value.upper() not in SQL_KEYWORDS and
                next_token.value.upper() not in ALL_SQL_FUNCTIONS):
                tokens[i + 1] = Token(TokenType.IDENTIFIER_ALIAS, next_token.value, next_token.space)
        
        # Detect implicit column aliases (identifier after column in SELECT)
        elif (token.type == TokenType.IDENTIFIER and 
              i > 0 and i + 1 < len(tokens)):
            ...
            if (prev_token.type in {TokenType.IDENTIFIER, TokenType.FUNCTION} and
                next_token.value in {',', 'FROM'} and
                token.value.upper() not in SQL_KEYWORDS and
                token.value.upper() not in ALL_SQL_FUNCTIONS):
    ...
    for i, token in enumerate(tokens):
        if (token.type == TokenType.IDENTIFIER and 
            token.value.lower() in table_aliases and
            i + 1 < len(tokens) and
            tokens[i + 1].type == TokenType.SYMBOL and
            tokens[i + 1].value == '.'):

...
def tokenize_sql(query: str) -> List[Token]:
    ...
    valid_specification = [
        (tt, pattern) for tt, pattern in token_specification
        if isinstance(tt, TokenType) and isinstance(pattern, str) and pattern.strip()
    ]
    # Combine patterns into a single regex
    token_regex = '|'.join(f'(?P<{tt.name}>{pattern})' for tt, pattern in valid_specification)
...
if __name__ == "__main__":
    sample_text = [
        ...
        " SELECT p.column  from table p where p.id = 10",
    ]

...
class TestArgumentParser:
    ...
    def test_file_options_parsing(self):
        ...
        args = parser.parse_args(
            ["-m", "custom.json", "anonymize", "-f", "input.sql", "-o", "output.sql"]
        )
    ...
    def test_global_options_parsing(self):
        ...
        args = parser.parse_args(
            ["--no-auto-save", "--verbose", "anonymize", "SELECT * FROM users"]
        )
...
class TestPersistence:
    ...
    def test_persistence_across_sessions(self, tmp_path):
        ...
        result1 = cli1.anonymize_query(
            "SELECT user_id FROM accounts", mapping_file=str(mapping_file)
        )
